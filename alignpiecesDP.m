function[X1n,q1n,X2n,q2n,q1l,q2ln,gam] = alignpiecesDP(X1,X2,Y1,Y2)

% Load some parameters, no need to change this
    lam = 0;
    N = 100;
    
% Converts each curve piece to q
    numPieces = size(Y1,2);
...
% Realigning pieces of curves, and putting pieces back together
    ...
    for j=1:numPieces
        ...
        len1(j) = trapz(linspace(0,1,N),sqrt(sum(v1.*v1)));
        len2(j) = trapz(linspace(0,1,N),sqrt(sum(v2.*v2)));
    end
